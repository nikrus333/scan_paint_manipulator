...
import time
from libs.system_defs import States, StateEvents, ModeCommands, Modes, InterpreterEvents, InterpreterStates
...
class RobotCommand(object):
    ...
    def toolTipOffset(self, tool_tip_offset):
        ...
        if self.manualJointMode():
            self.__req.setParameterList([
                {
                    "path": "root/ManipulatorControl/mechanismManager/tt_offset",
                    "value": tool_tip_offset
                },
                {
                    "path": "root/ManipulatorControl/mechanismManager/update_counter",
    ...
    def moveToPoint(self, target_joint_coord_rad, v_max=0.5, a_max=1.0):
        ...
        if self.semiAutoMode():
            self.__req.setParameterList([
                {
                    "path": "root/ManipulatorControl/motionGenerators/semiAutoMaxAcc",
                    "value": a_max
                },
                {
                    "path": "root/ManipulatorControl/motionGenerators/semiAutoMaxVel",
                    "value": v_max
                },
                {
                    "path": "root/ManipulatorControl/motionGenerators/semiAutoUserTarget",
                    "value": target_joint_coord_rad
                }]).get()

            self.__req.setParameter(
                "root/ManipulatorControl/activateSemiAuto", True).get()
            while True:
                motion_not_done = \
                    self.__req.getParameter(
                        "root/ManipulatorControl/motionGenerators/semiAutoGenerator/motionStateOut").get().value[0]
                if not motion_not_done:
                    self.__req.setParameter(
                        "root/ManipulatorControl/activateSemiAuto", False).get()
                    self.autoMode()
                    return True
                time.sleep(0.1)
    ...
    def moveToStart(self, timeout_s):
        ...
        while True:
            counter += 1
            self.__req.setParameter("root/ManipulatorControl/activateMoveToStart", 1)
            self.__req.setParameter("root/ManipulatorControl/activateSemiAuto",
                                    1).get()
